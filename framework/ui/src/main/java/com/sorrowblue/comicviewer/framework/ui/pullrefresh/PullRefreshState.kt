package com.sorrowblue.comicviewer.framework.ui.pullrefresh
 ...
 *
 * @param refreshing A boolean representing whether a refresh is currently
 *     occurring.
 * @param onRefresh The function to be called to trigger a refresh.
 * @param refreshThreshold The threshold below which, if a release occurs,
 *     [onRefresh] will be called.
 * @param refreshingOffset The offset at which the indicator will be drawn
 *     while refreshing. This offset corresponds to the position of the
...
/**
 * A state object that can be used in conjunction with [pullRefresh] to
 * add pull-to-refresh behaviour to a scroll component. Based on Android's
 * SwipeRefreshLayout.
 *
 * Provides [progress], a float representing how far the user has pulled
 * as a percentage of the refreshThreshold. Values of one or less indicate
 * that the user has not yet pulled past the threshold. Values greater
 * than one indicate how far past the threshold the user has pulled.
 *
 * Can be used in conjunction with [pullRefreshIndicatorTransform]
 * to implement Android-like pull-to-refresh behaviour with a custom
 * indicator.
 *
...
class PullRefreshState internal constructor(
     ...
     * refreshThreshold.
     *
     * If the component has not been pulled at all, progress is zero. If
     * the pull has reached halfway to the threshold, progress is 0.5f.
     * A value greater than 1 indicates that pull has gone beyond the
     * refreshThreshold - e.g. a value of 2f indicates that the user has pulled
     * to two times the refreshThreshold.
     */
...
object PullRefreshDefaults {
    /**
     * If the indicator is below this threshold offset when it is released, a
     * refresh will be triggered.
     */
    val RefreshThreshold = 80.dp

    /**
     * The offset at which the indicator should be rendered whilst a refresh is
     * occurring.
     */
...
/**
 * The distance pulled is multiplied by this value to give us the adjusted
 * distance pulled, which is used in calculating the indicator position
 * (when the adjusted distance pulled is less than the refresh threshold,
 * it is the indicator position, otherwise the indicator position is
