...
internal class HistorySerializer(private val coroutineDispatcher: CoroutineDispatcher = Dispatchers.IO) :
    Serializer<History> {
    override val defaultValue: History = History()
    override suspend fun readFrom(input: InputStream): History {
        return ProtoBuf.decodeFromByteArray(History.serializer(), input.readBytes())
    }

    override suspend fun writeTo(t: History, output: OutputStream) {
        withContext(coroutineDispatcher) {
            @Suppress("BlockingMethodInNonBlockingContext")
            output.write(ProtoBuf.encodeToByteArray(History.serializer(), t))
        }
    }
}
